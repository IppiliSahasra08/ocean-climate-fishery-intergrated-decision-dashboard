...
# GFW Region IDs (Example IDs, should be replaced with actual EEZ/RFMO IDs)
REGION_TO_GFW_REGION = {
    "bay_of_bengal": "8480",  # Indian EEZ
    "north_atlantic": "8456", # USA EEZ
    "eastern_pacific": "8486" # Mexican EEZ
}
...
async def fetch_gfw_data(region_key: str) -> dict:
    ...
    # GFW V3 requires specific parameter names and array indexing
    params = [
        ("format", "JSON"),
        ("group-by", "GEARTYPE"),
        ...
        ("region-id", region_id),
        ("region-dataset", "public-eez-v8")
    ]

    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
            print(f"Requesting GFW stats for {region_key} with params: {params}")
            response = await client.get(url, headers=headers, params=params)
            
            # If GFW fails (e.g., 422, 404, 401), we log and fallback to simulated data
            # this prevents the whole endpoint from failing if GFW has schema/access issues
            if response.status_code != 200:
                print(f"GFW API Error {response.status_code}. Using simulated data fallback.")
                with open("gfw_error.log", "w") as f:
                    f.write(f"Status: {response.status_code}\nBody: {response.text}")
                
                # Randomized simulated data for demo purposes when API fails
                import random
                ...
                if isinstance(data, list):
                    for row in data:
                        total_fishing_hours += float(row.get("apparent_fishing_hours") or row.get("value") or 0)
                elif isinstance(data, dict):
                    total_fishing_hours = data.get("total_apparent_fishing_hours", 500)
